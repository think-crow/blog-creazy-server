...
router.get('/bookmovies_data',async function(req, res, next) {
   ...
   const total = await BookMoviesModel.countDocuments(query);
        if (page > total) page = 1;
                 await BookMoviesModel.find(query).skip((page - 1) * limit).limit(limit).then((result)=>{  
...
// 后台请求书影数据
router.get('/bookmovies_alldata',async function(req, res, next) {
  ...
  await BookMoviesModel.find(query).then((result)=>{  
    res.json(result);
 }).catch((err)=>{
...
// 删除一条
router.delete('/deleteone_bookmovies/:_id',async function(req, res, next) {
