...
router.get('/notepapers_data',async function(req, res, next) {
   ...
   const total = await NotePapersModel.countDocuments(query);
        if (page > total) page = 1;
        if (limit > total) limit = total;
        await NotePapersModel.find(query).skip((page - 1) * limit).limit(limit).then((result)=>{  
...
// // 后台请求书影数据
router.get('/notepapers_alldata',async function(req, res, next) {
  ...
  await NotePapersModel.find(query).then((result)=>{  
    res.json(result);
 }).catch((err)=>{
...
// // 删除一条
router.delete('/deleteone_notepapers/:_id',async function(req, res, next) {
...
// // 更新一条数据
// // 定义 PATCH 路由用于更新数据
router.patch('/notepapers_updata_one',async (req, res) => {
  try {
    ...
    // 使用 Mongoose 更新文档数据
    const result = await NotePapersModel.findByIdAndUpdate(_id, updatedData, { new: true });
    res.json(result);
  } catch (error) {
    console.error(error);
...
// 添加一条数据
router.post('/notepapers', (req, res) => {
