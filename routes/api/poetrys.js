...
router.get('/poetry_data',async function(req, res, next) {
   ...
   const total = await PoetryModel.countDocuments(query);
        ...
        await PoetryModel.find(query).skip((page - 1) * limit).limit(limit).then((result)=>{  
...
// 后台请求书影数据
router.get('/poetry_alldata',async function(req, res, next) {
  ...
  await PoetryModel.find(query).then((result)=>{  
    res.json(result);
 }).catch((err)=>{
...
// 删除一条
router.delete('/deleteone_poetry/:_id',async function(req, res, next) {
...
// 更新一条数据
// 定义 PATCH 路由用于更新数据
router.patch('/poetry_updata_one',async (req, res) => {
  try {
